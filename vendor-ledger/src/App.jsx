...
const syncEditLog = async (log) => {
  try {
    const params = new URLSearchParams({ action: "write_edit_log", data: JSON.stringify(log) });
    await fetch(`${SCRIPT_URL}?${params.toString()}`, { mode: "no-cors" });
  } catch (_) {}
};
...
const recordLabel = (r) => `${r.date} | ${r.vendor} | ${r.content} | ${r.type==="in"?"收入":"支出"} ${r.type==="in"?"+":"-"}${r.amount}`;
...
export default function App() {
  ...
  const [authRecord, setAuthRecord] = useState(null); // record being edited
  ...
  const [editAuth, setEditAuth] = useState(null); // { name, reason }
  ...
  const withBalance = (recs) => {
    let bal = 0;
    return recs.map(r => { bal += r.type === "in" ? r.amount : -r.amount; return { ...r, balance: bal }; });
  };
  ...
  // Step 1: Click 編輯 → show auth modal
  const clickEdit = (rec) => {
    setAuthRecord(rec);
    setAuthName("");
    setAuthReason("");
  ...
  // Step 2: Submit auth → open edit form
  const submitAuth = () => {
  ...
  // Step 3: Save edit → log to sheets
  const submitEdit = async () => {
    ...
    const log = {
      ...
      editor: editAuth.name,
      reason: editAuth.reason,
      original: recordLabel(original),
      updated: recordLabel(updated_rec),
    };
    await syncEditLog(log);
    setSyncStatus("ok");
  ...
  const submitAdd = async () => {
    ...
    setAddForm({ date: today(), vendor: "鼎耀", content: "", type: "out", amount: "", receipt: false });
    setSyncStatus("syncing");
  ...
  const focusStyle = "input:focus,select:focus,textarea:focus{border-color:#3dff7e!important}";

  return (
    <div style={{ background:"#0f0f0f", minHeight:"100vh", color:"#e8e8e8", fontFamily:"'Courier New', monospace" }}>
      <style>{`* { box-sizing: border-box; } ${focusStyle} button{cursor:pointer;font-family:'Courier New',monospace;transition:all .15s;} button:active{transform:scale(.96);} tr:hover td{background:#161616;}`}</style>
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.8)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200 }}
          ...
          <div style={{ background:"#141414", border:"1.5px solid #ff6b6b", borderRadius:10, padding:"28px 32px", width:440, maxWidth:"90vw" }}>
            ...
            <div style={{ fontSize:11, color:"#555", marginBottom:20, padding:"8px 12px", background:"#1a1a1a", borderRadius:4, lineHeight:1.7 }}>
              {authRecord && recordLabel(authRecord)}
            </div>

            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:10, color:"#888", letterSpacing:2, marginBottom:5 }}>修改人姓名 <span style={{color:"#ff6b6b"}}>*</span></div>
              <input value={authName} onChange={e=>{setAuthName(e.target.value);setAuthError("");}} placeholder="請輸入您的姓名" style={inputStyle} onKeyDown={e=>e.key==="Enter"&&document.getElementById("auth-reason")?.focus()} />
            </div>
            ...
            <div style={{ marginBottom:authError?10:20 }}>
              ...
              <textarea id="auth-reason" value={authReason} onChange={e=>{setAuthReason(e.target.value);setAuthError("");}} placeholder="請說明為什麼需要修改這筆記錄..." rows={3} style={{ ...inputStyle, resize:"vertical" }} />
            </div>
      ...
      {showEditForm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.8)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:200 }}>
          <div style={{ background:"#141414", border:"1px solid #2a2a2a", borderRadius:10, padding:"28px 32px", width:460, maxWidth:"90vw" }}>
            <div style={{ fontSize:11, color:"#f5c542", letterSpacing:3, marginBottom:4 }}>編輯記錄</div>
            <div style={{ fontSize:11, color:"#555", marginBottom:18, padding:"6px 10px", background:"#1a1a1a", borderRadius:4 }}>
              修改人：<span style={{color:"#f5c542"}}>{editAuth?.name}</span>　原因：{editAuth?.reason}
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
              ...
              <div>
                ...
                <select value={form.vendor} onChange={e=>setForm(f=>({...f,vendor:e.target.value}))} style={inputStyle}>
                  {VENDORS.map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
            </div>
            ...
            <div style={{ marginTop:14 }}>
              ...
              <input value={form.content} onChange={e=>setForm(f=>({...f,content:e.target.value}))} style={inputStyle} onKeyDown={e=>e.key==="Enter"&&amountRef.current?.focus()} />
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginTop:14 }}>
              ...
              <div>
                ...
                <input ref={amountRef} type="number" value={form.amount} onChange={e=>setForm(f=>({...f,amount:e.target.value}))} placeholder="0" style={inputStyle} onKeyDown={e=>e.key==="Enter"&&submitEdit()} />
              </div>
            </div>
      ...
      {showAddForm && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.75)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:100 }}
          ...
          <div style={{ background:"#141414", border:"1px solid #2a2a2a", borderRadius:10, padding:"28px 32px", width:460, maxWidth:"90vw" }}>
            <div style={{ fontSize:14, fontWeight:700, marginBottom:20, letterSpacing:1 }}>新增記錄</div>

            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
              <div>
                <div style={{ fontSize:10, color:"#555", letterSpacing:2, marginBottom:5 }}>日期</div>
                <input type="date" value={addForm.date} onChange={e=>setAddForm(f=>({...f,date:e.target.value}))} style={{ ...inputStyle, colorScheme:"dark" }} />
              </div>
              <div>
                <div style={{ fontSize:10, color:"#555", letterSpacing:2, marginBottom:5 }}>廠商</div>
                <select value={addForm.vendor} onChange={e=>setAddForm(f=>({...f,vendor:e.target.value}))} style={inputStyle}>
                  {VENDORS.map(v => <option key={v} value={v}>{v}</option>)}
                </select>
              </div>
            </div>
            ...
            <div style={{ marginTop:14 }}>
              <div style={{ fontSize:10, color:"#555", letterSpacing:2, marginBottom:5 }}>內容</div>
              <input value={addForm.content} onChange={e=>setAddForm(f=>({...f,content:e.target.value}))} placeholder="例：啤酒 x24、本月薪資..." style={inputStyle} onKeyDown={e=>e.key==="Enter"&&addAmountRef.current?.focus()} />
            </div>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14, marginTop:14 }}>
              ...
              <div>
                <div style={{ fontSize:10, color:"#555", letterSpacing:2, marginBottom:5 }}>金額</div>
                <input ref={addAmountRef} type="number" value={addForm.amount} onChange={e=>setAddForm(f=>({...f,amount:e.target.value}))} placeholder="0" style={inputStyle} onKeyDown={e=>e.key==="Enter"&&submitAdd()} />
              </div>
            </div>
